...
const observer = new IntersectionObserver(index => {
    index.forEach(visivel => {
        if (visivel.isIntersecting) {
            visivel.target.style.opacity = 1;
        } else {
            visivel.target.style.opacity = 0;
        }
    })
})
const elemento = document.querySelectorAll('.secao')
elemento.forEach(el => {
    observer.observe(el)
})
